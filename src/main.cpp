...
uint256 hashGenesisBlock(
        "0x9677102b58bd0d0888deadf88cf15a3e6e32b0b1c66eacefc668126514995cb3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Guarantcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Guarantcoin Signed Message:\n";
...
bool CTxOut::IsDust() const {
    // Guarantcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Guarantcoin: 3.5 days
static const int64 nTargetSpacing = 3 * 60; // Guarantcoin: 3 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast,
    ...
    // Guarantcoin: This fixes an issue where a 51% attack can change difficulty at will.
    ...
    int64 nActualTimespan = pindexLast->GetBlockTime()
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast,
    ...
    for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
        ...
        if (LatestBlockTime < BlockReading->GetBlockTime()) {
            if (BlockReading->nHeight > 0) // HARD Fork block number
...
unsigned int static GetNextWorkRequired_V2(const CBlockIndex* pindexLast,
    ...
    int64 PastSecondsMin = TimeDaySeconds * 0.3;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast,
        const CBlockHeader *pblock) {
    int DiffMode = 2; // legacy diff-mode
    if (fTestNet) {
        if (pindexLast->nHeight + 1 >= 2116) {
            DiffMode = 2;
        } // guarantcoin, 100 blocks after first legacy diff adjustment
    } else {
        if (pindexLast->nHeight + 1 >= 26754) {
            DiffMode = 2;
        } //guarantcoin, 5 days after 27/01/2014 12:00 UTC
    }
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex,
    ...
    for (unsigned int i = 0; i < vtx.size(); i++) {
        ...
        if (!tx.IsCoinBase()) {
            ...
            if (!tx.CheckInputs(state, view, fScriptChecks, flags,
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW,
    ...
    // Guarantcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool CBlockIndex::IsSuperMajority(int minVersion, const CBlockIndex* pstart,
    ...
    // Guarantcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex() {
    if (fTestNet) {
        ...
        hashGenesisBlock =
                uint256(
                        "0x282233a4ad146321879a67ba253f5f9d4a134a76455f0200ab9079faa0dc6d06");
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "03/23/2014 Czech in a shadow of Krym events";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce = 351254;
        block.nTime = 1396381854;

        if (fTestNet) {
            block.nNonce = 12603389;
            block.nTime = 1395583953;
        }
        ...
        assert(
                block.hashMerkleRoot
                        == uint256(
                                "0xa39c89f12204e5ea060fbed4b6c784cd7be6166ce58f248be6338c6a16b3980d"));
        ...
        if (false && block.GetHash() != hashGenesisBlock) {
            ...
            uint256 thash;
            unsigned long int scrypt_scratpad_size_current_block = ((1
                    << (GetNfactor(block.nTime) + 1)) * 128) + 63;

            char scratchpad[scrypt_scratpad_size_current_block];

            loop {
                scrypt_N_1_1_256_sp_generic(BEGIN(block.nVersion), BEGIN(thash),
                        scratchpad, GetNfactor(block.nTime));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey) {
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GuarantcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GuarantcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GuarantcoinMiner(CWallet *pwallet) {
    printf("GuarantcoinMiner started\n");
    SetThreadPriority (THREAD_PRIORITY_LOWEST);
    RenameThread("guarantcoin-miner");
    ...
    try {
        loop {
            ...
            printf(
                    "Running GuarantcoinMiner with %"PRIszu" transactions in block (%u bytes)\n",
                    pblock->vtx.size(),
            ...
            loop {
                ...
                unsigned long int scrypt_scratpad_size_current_block = ((1
                        << (GetNfactor(pblock->nTime) + 1)) * 128) + 63;
                ...
                loop {
                    ...
                    // Generic scrypt
                    scrypt_N_1_1_256_sp_generic(BEGIN(pblock->nVersion),
        ...
        printf("GuarantcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet) {
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GuarantcoinMiner, pwallet));
}
